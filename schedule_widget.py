...
from schedule_utils_ui_render import setup_hebrew_locale, format_tasks, get_sleep_message, render_no_tasks_message, format_schedule_output
...
def generate_schedule_display():
    ...
    if (msg := get_sleep_message(now)):
        results.append(msg)
    schedule, tasks_to_display = load_and_filter_schedule(now)
    if not schedule:
        return render_no_tasks_message(now)
    current_task_index = find_current_task_index(tasks_to_display, now)
    results += format_tasks(tasks_to_display, now, current_task_index)
